    ...
    for (s=num;s<select.length;s++){
       select[s].innerHTML = ''
        ...
        for (i=0;i<dados.length;i++){
            ...
            opt.value = dados[i]['qid'];
            opt.innerHTML = dados[i]['question'];
...
document.querySelector('#select').addEventListener('change', () => {
    ...
    innputEdit.value = dados[question]['question'];
    ...
    let respostas = JSON.parse(dados[question]['answers']);
    let prox = JSON.parse(dados[question]['nextid']);
    id = dados[question]['qid'];
    ...
    for (i=0;i<respostas.length;i++){
        ...
        for (d=0;d<dados.length;d++){
            ...
            opt.value = dados[d]['qid'];
            opt.innerHTML = dados[d]['question'];
    ...
    if (pergunta.length != 0){
        fetch("http://localhost:8080/perguntas",
        {
            ...
            body: JSON.stringify({
                "qid":id,
                "question": pergunta,
                "answers": JSON.stringify(respJ),
                "nextid":  JSON.stringify(nextID)
